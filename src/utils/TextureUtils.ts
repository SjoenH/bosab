...
export class TextureUtils {
     ...
     * @returns A THREE.CanvasTexture.
     */
    public static createCharacterTexture(
        char: string,
        config: {
            canvasWidth: number;
            canvasHeight: number;
        ...
        if (config.trailCount && config.trailOpacityStep) {
            ...
            if (baseFillStyle.startsWith('rgba')) {
                const parts = baseFillStyle.match(/rgba\\((\\d+),\\s*(\\d+),\\s*(\\d+),\\s*([\\d.]+)\\)/);
                if (parts) {
            ...
            for (let i = 1; i < config.trailCount; i++) {
                ...
                if (baseColor.startsWith('rgb(')) { // was originally rgb or rgba
                    context.fillStyle = baseColor.replace('rgb(', 'rgba(').replace(')', `, ${opacity})`);
                } else { // was hex or named, need to convert to rgba. This is a simplification.
     ...
     * @returns A THREE.CanvasTexture.
     */
    public static createStreakTexture(config: {
        canvasSize: number;
        ...
        const gradient = context.createLinearGradient(
            config.rect.x, 0, config.rect.x + config.rect.width, 0 // Horizontal gradient
        );
        ...
        context.fillStyle = gradient;
        context.fillRect(config.rect.x, config.rect.y, config.rect.width, config.rect.height);
