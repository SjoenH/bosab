...
export class Act1Matrix extends BaseAct {
    ...
    private waveformMaterial: THREE.LineBasicMaterial = new THREE.LineBasicMaterial();
    private waveformPoints: number = 100;
    ...
    protected async createContent(): Promise<void> {
        ...
        this.waveformGeometry.setAttribute('position', new THREE.BufferAttribute(waveformPositions, 3));
        ...
        this.waveformLine = new THREE.Line(this.waveformGeometry, this.waveformMaterial);
    ...
    protected updateContent(deltaTime: number): void {
        ...
        for (let i = 0; i < this.particleCount; i++) {
            ...
            positions[i3] += Math.sin(this.time * 0.001 + i * 0.1) * trebleLevel * 0.02;
