...
export class Act4Stars extends BaseAct {
    private starCount: number = 3000;
    private nebulaCount: number = 200;
    ...
    private originalPositions: Float32Array = new Float32Array(this.starCount * 3);
    private nebulaPositions: Float32Array = new Float32Array(this.nebulaCount * 3);
    ...
    private driftAmplitude: number = 5;
    private driftSpeed: number = 0.0001;
    ...
    protected async createContent(): Promise<void> {
        ...
        for (let i = 0; i < this.nebulaCount; i++) {
            ...
            const i3 = i * 3;
            nebulaPositions[i3] = this.nebulaPositions[i3] = radius * Math.sin(phi) * Math.cos(theta);
            nebulaPositions[i3 + 1] = this.nebulaPositions[i3 + 1] = radius * Math.sin(phi) * Math.sin(theta);
            nebulaPositions[i3 + 2] = this.nebulaPositions[i3 + 2] = radius * Math.cos(phi);
        ...
        this.nebulaGeometry.setAttribute('position', new THREE.BufferAttribute(nebulaPositions, 3));
        this.nebulaGeometry.setAttribute('color', new THREE.BufferAttribute(this.nebulaColors, 3));
    ...
    protected updateContent(deltaTime: number): void {
        ...
        const nebulaPositions = this.nebulaGeometry.attributes.position.array as Float32Array;
        ...
        for (let i = 0; i < this.nebulaCount; i++) {
            ...
            const angle = Math.atan2(this.nebulaPositions[i3 + 1], this.nebulaPositions[i3]);
            ...
            nebulaPositions[i3 + 2] = this.nebulaPositions[i3 + 2] + Math.sin(this.nebulaPhases[i]) * 0.02;
        }
