...
export class Controls {
	...
	public init(): void {
		...
		// Timing configuration elements
		this.actDurationInput = document.getElementById(
			"actDuration",
		) as HTMLInputElement;
		this.transitionDurationInput = document.getElementById(
			"transitionDuration",
		) as HTMLInputElement;
		this.demoActDurationInput = document.getElementById(
			"demoActDuration",
		) as HTMLInputElement;
		this.demoTransitionDurationInput = document.getElementById(
	...
	private updateTimingDisplays(): void {
		...
		if (this.actDurationInput) {
			const actDuration =
				Number.parseFloat(this.actDurationInput.value) || 6.25;
			const displaySpan = this.actDurationInput.parentNode?.querySelector(
				".duration-display",
			) as HTMLElement;
			if (displaySpan) {
		...
		if (this.transitionDurationInput) {
			const transitionDuration =
				Number.parseFloat(this.transitionDurationInput.value) || 3;
			const displaySpan =
				this.transitionDurationInput.parentNode?.querySelector(
					".duration-display",
				) as HTMLElement;
			if (displaySpan) {
		...
		if (this.demoActDurationInput) {
			const demoActDuration =
				Number.parseFloat(this.demoActDurationInput.value) || 5;
			const displaySpan = this.demoActDurationInput.parentNode?.querySelector(
				".duration-display",
			) as HTMLElement;
			if (displaySpan) {
		...
		if (this.demoTransitionDurationInput) {
			const demoTransitionDuration =
				Number.parseFloat(this.demoTransitionDurationInput.value) || 0.5;
			const displaySpan =
				this.demoTransitionDurationInput.parentNode?.querySelector(
					".duration-display",
				) as HTMLElement;
			if (displaySpan) {
	...
	private updateTotalDurations(): void {
		const actDuration = Number.parseFloat(
			this.actDurationInput?.value || "6.25",
		);
		const transitionDuration = Number.parseFloat(
			this.transitionDurationInput?.value || "3",
		);
		const demoActDuration = Number.parseFloat(
			this.demoActDurationInput?.value || "5",
		);
		const demoTransitionDuration = Number.parseFloat(
			this.demoTransitionDurationInput?.value || "0.5",
		);
	...
	private applyTimingSettings(): void {
		...
		this.showMessage(
			`Timing updated: ${actDuration}min acts, ${demoActDuration}s demo`,
			2000,
		);
	}
	...
	public getTimingConfig(): TimingConfig {
		return {
			actDuration: Number.parseFloat(this.actDurationInput?.value || "6.25"),
			transitionDuration: Number.parseFloat(
				this.transitionDurationInput?.value || "3",
			),
			demoActDuration: Number.parseFloat(
				this.demoActDurationInput?.value || "5",
			),
			demoTransitionDuration: Number.parseFloat(
				this.demoTransitionDurationInput?.value || "0.5",
			),
		};
	}
